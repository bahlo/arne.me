...
async function parseIssues() {
  ...
  const parsed = await Promise.all(files.filter(filename => filename != "_index.md" && filename.endsWith('.md')).map(async filename => {
    const slug = filename.substring(0, filename.length - 3);
    const source = await fs.readFile(buildAbsolutePath('content/weekly/'+filename))
